...
/*void Aplicatie::setare(const Aplicatie app) {
	this->setNume(app.getNume());
	this->setNrImnatriculare(app.getNrInmatriculare());
...
/*ostream& operator<<(ostream& os, Aplicatie e) {
	os << e.nume << " " << e.numarInmatriculare << " " << e.status << endl;
	return os;
}

istream& operator >>(istream& is, Aplicatie& e) {
	cout << "Introduceti numele posesorului auto: ";
	char* name = new char[20];
	is >> name;
	cout << "Numarul de inmatriculare: ";
	char* numar = new char[20];
	is >> numar;
	cout << "Liber sau ocupat: ";
	char* status = new char[10];
	int ok = 0;

	e.setNume(name);
	e.setNrInmatriculare(numar);
	e.setStatus(status);
	delete[] name;
	delete[] numar;
	delete[] status;
